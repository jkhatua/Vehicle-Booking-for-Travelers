...
function fetchcarcharges($vehicle_id1,$charging_type1){
    ...
    ");
       ...
       while($stmt->fetch()){
         $row1=array('id'=>$id,'vehicle_id'=>$vehicle_id,'vehicle_name'=>$vehicle_name,'charging_type'=>$charging_type,'day_charge'=>$day_charge,'night_holt_charge'=>$night_holt_charge,'km_cover_in_one_litre'=>$km_cover_in_one_litre,'extra_hour_charges'=>$extra_hour_charges,'price_per_km'=>$price_per_km);
       }
       $stmt->close();
      if(!empty($row1)){
      return ($row1);
