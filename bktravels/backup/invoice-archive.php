...
function fetchProduct(){
  ...
  ");
  $stmt->execute();
  $stmt->bind_result($id,$order_no,$invoice_no,$customer_details,$mat_des,$amount,$tax_cost,$rdate,$pay_cost,$bill,$quantity);
  while($stmt->fetch()){
    $row[] = array('id'=>$id,'order_no' =>$order_no ,'invoice_no'=>$invoice_no,'customer_details'=>$customer_details,'mat_des'=>$mat_des,'amount'=>$amount,'tax_cost'=>$tax_cost,'rdate'=>$rdate,'pay_cost'=>$pay_cost,'bill'=>$bill,'quantity'=>$quantity );
  }
  $stmt->close();
	if(!empty($row))
	{
