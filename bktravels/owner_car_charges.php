  ...
  function fetchownercarcharges($status){
    ...
    $stmt = $mysqli->prepare("SELECT
      ...
      WHERE status = ?
       ...
       $stmt->execute();
       $stmt->bind_result($id,$vehicle_name,$charging_type,$day_charge,$night_holt_charge,$km_cover_in_one_litre,$extra_hour_charges,$price_per_km);
       while($stmt->fetch()){
         $row[]=array('id'=>$id,'vehicle_name'=>$vehicle_name,'charging_type'=>$charging_type,'day_charge'=>$day_charge,'night_holt_charge'=>$night_holt_charge,'km_cover_in_one_litre'=>$km_cover_in_one_litre,'extra_hour_charges'=>$extra_hour_charges,'price_per_km'=>$price_per_km);
       }
       $stmt->close();
     	if(!empty($row)){
     	return ($row);
...
<?php include("include/header.php"); ?>
  ...
  <script src="vendors/bower_components/sweetalert/dist/sweetalert.min.js"></script>
	...
	function edit(val) {
		$.ajax({
		type: "POST",
		url: "ajax_edit_owner_car_charges.php",
		data:'id='+val,
		...
		function delete_buyer(val){
			$.ajax({
			type: "POST",
			url: "ajax_delete_owner_car_charges.php",
			data:'id='+val,
			success: function(data){
